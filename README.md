...
- **カスタムダッシュボード**: 時計・天気・カメラ映像・死活モニターなど 10 種類のウィジェットをドラッグ＆リサイズで自由に配置
...
デバイス名・メモ・ダッシュボード構成・設定は **すべて 1 ファイル**（`data/db.json`）に入っています。

- 画面から: 設定 → 「⬇️ バックアップをダウンロード」
- コマンドで（cron の例・毎日3時に曜日ごと7世代保持）:

```bash
# crontab -e
0 3 * * * cp /opt/iot-dashboard/data/db.json /opt/iot-dashboard/data/backup-$(date +\%u).json
```

復元はサービスを止めて `db.json` を書き戻すだけです。
...
| `GET /api/export` | 全データのバックアップダウンロード |
...
server/               Node.js（標準ライブラリのみ）
  ...
  lib/{wol,oui,system,weather,demo}.js
